...
tcp_select (volatile struct sock *sk, int sel_type, select_table *wait)
{
	switch (sel_type)
	{
		case SEL_IN:
			...
			if (sk->rqueue != NULL &&
			    (between (sk->copied_seq, sk->rqueue->next->h.th->seq - 1, sk->rqueue->next->h.th->seq + sk->rqueue->next->len) ||
			     sk->state == TCP_LISTEN))
			{
				return (1);
			}
	      ...
	      struct tcp_header *th, unsigned long daddr)
{
	...
	/* we need to grab some memory, and put together an ack, and then put it into the queue to be sent. */
	 ...
	 int len, int nonblock, unsigned flags)
{
	...
	while ( len > 0)
	{
		...
		while (skb == NULL || before (sk->copied_seq+1, skb->h.th->seq) || skb->used)
		{
			...
			if ( sk->rqueue == NULL || before (sk->copied_seq+1, sk->rqueue->next->h.th->seq))
			{
				interruptible_sleep_on (sk->sleep);
				if (current->signal & ~current->blocked)
				{
		...
		/* Copy anything from the current block that needs to go into the user buffer. */
		/* 获取要拷贝到用户态的数据在skb中的偏移量 */
		...
		if (offset < skb->len)
		{
			/* if there is urgent data we must either return or skip over it. */
			if (skb->h.th->urg)
			{
				if (skb->urg_used)
				{
...
/* This routine handles a connection request.  This should make sure we haven't already responded. */
/* Because of the way BSD works, we have to send a syn/ack now. This also means it will be harder to close a socket which is listening. */
	...
	unsigned long saddr, int redo, struct ip_protocol * protocol)
{
	...
	if (!redo)
	{
		/* 如果校验和检查没通过，则释放skb，检查通过此时tcp_check()应该返回0 */
		if (th->check && tcp_check (th, len, saddr, daddr ))
		{
			skb->sk = NULL;
			free_skb (skb, 0);
			/* we don't release the socket because it was never marked in use. */
			return (0);
		}

		/*See if we know about the socket. */
		/* 找不到对应的socket，则回复RST，然后释放skb */
		if (sk == NULL)
		{
			if (!th->rst)
...
tcp_write_wakeup(volatile struct sock *sk)
{
	...
	t1 = (struct tcp_header *)(buff + 1);		//这部分代码没用!!!
